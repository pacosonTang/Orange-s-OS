...
SetupPaging:
	...
	; 首先初始化页目录
	; SelectorPageDir		equ	LABEL_DESC_PAGE_DIR	- LABEL_GDT
	mov	ax, SelectorPageDir	; 此段首地址为 PageDirBase ; PageDirBase equ 200000h	; 页目录开始地址: 2M
	mov	es, ax
...
.1: ; 这里是在初始化页目录中项的内容，即对应页表的内存地址
	stosd  ; stosb, stosw, stosd 把al/ ax/ eax的内容存储到 es:edi 指向的内存单元中, 该指令执行后，edi自增1,es=PageDirBase
	add	eax, 4096		; 为了简化, 所有页表在内存中是连续的.，每个页表占用4k字节空间 
	loop	.1
	; 初始化页目录的项内容 over
	
	; 再初始化所有页表 (1K 个, 4M 内存空间)的项内容，即页表的项存储的是内存地址的高20位地址；
	mov	ax, SelectorPageTbl	; 此段首地址为 PageTblBase equ 201000h
	mov	es, ax
...
.2:
	stosd ; 初始值eax=0111, eax += 4096，eax -> es:edi ;  es:edi++[PageTblBase:edi++]
