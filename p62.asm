...
LABEL_DESC_CODE_DEST:  Descriptor 0, SegCodeDestLen-1, DA_C+DA_32; 非一致代码段,32 ( 98h + 4000h )
...
LABEL_DESC_LDT:        Descriptor 0,         LDTLen-1, DA_LDT    ; LDT; DA_LDT=82h,DPL=0	; 局部描述符表段类型值
; 任务状态段描述符 LABEL_DESC_TSS [add]
...
LABEL_SEG_CODE_DEST:
	...
	; 以下几行代码的说明：（Load LDT 和 retf 二选一 即可） [add]
	; Load LDT段是在 调用门目标代码段执行完后，返回到 实模式
	; 而 retf 是直接返回，不返回到实模式
...
LABEL_CODE_RING3:
	...
	call	SelectorCallGateTest:0 ; 打印 'C'
