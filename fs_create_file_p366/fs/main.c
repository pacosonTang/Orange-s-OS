...
PUBLIC struct inode * get_inode(int dev, int num)
{
	...
	for (p = &inode_table[0]; p < &inode_table[NR_INODE]; p++) {
		if (p->i_cnt) {	/* not a free slot 如果文件已经被打开了（该inode已经被其他进程使用了）*/
			if ((p->i_dev == dev) && (p->i_num == num)) {
				/* this is the inode we want */
				p->i_cnt++; //又多了一个进程共享 该inode p了；
				return p;
			}
		}
		else {		/* a free slot */
	...
	int blk_nr = 1 + 1 + sb->nr_imap_sects + sb->nr_smap_sects +
		((num - 1) / (SECTOR_SIZE / INODE_SIZE)); // 该inode数据结构所占的扇区编号（boot sector为起点）
	RD_SECT(dev, blk_nr); // 以blk_nr 为目标扇区，读取该扇区内容 到 fsbuf 缓冲区中；
	struct inode * pinode = // 该inode数据结构地址 相对于 该扇区的偏移量（在缓冲区 fsbuf 中）
...
PUBLIC void sync_inode(struct inode * p)
{
	...
	int blk_nr = 1 + 1 + sb->nr_imap_sects + sb->nr_smap_sects + 
		((p->i_num - 1) / (SECTOR_SIZE / INODE_SIZE));// 该inode数据结构所占的扇区编号（boot sector为起点）
	RD_SECT(p->i_dev, blk_nr);   // 以p->i_dev为驱动器号（指定是哪个硬盘） 和 blk_nr 为目标扇区，读取该扇区内容 到 fsbuf 缓冲区中；
	pinode = (struct inode*)((u8*)fsbuf +  			 // 该inode数据结构地址 相对于 该扇区的偏移量
	...
	WR_SECT(p->i_dev, blk_nr);  // 这里是从fsbuf 写入到 硬盘的某个分区（p->i_dev）的扇区blk_nr 中去；
}
