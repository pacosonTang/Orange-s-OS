...
PRIVATE void new_dir_entry(struct inode *dir_inode,int inode_nr,char *filename)
{
	...
	int i, j;
	for (i = 0; i < nr_dir_blks; i++) { //nr_dir_blks == 已分配的根目录所占扇区数目
		RD_SECT(dir_inode->i_dev, dir_blk0_nr + i); // 这里读，是从i_dev分区的 第 dir_blk0_nr + i 个扇区中读取内容到 fsbuf；

		pde = (struct dir_entry *)fsbuf;
		for (j = 0; j < SECTOR_SIZE / DIR_ENTRY_SIZE; j++,pde++) { //SECTOR_SIZE / DIR_ENTRY_SIZE==一个扇区有多少个根目录项；
			if (++m > nr_dir_entries)
				break;

			if (pde->inode_nr == 0) { /* it's a free slot == 有空闲的根目录项 */
				new_de = pde;
				break;
			}
		}
	...
	if (!new_de) { /* reached the end of the dir  ， 如果已分配的根目录项用完*/
		new_de = pde; // 在dir_entry 末尾追加一个根目录项
		dir_inode->i_size += DIR_ENTRY_SIZE; //根目录的字节大小自加；
	}
	new_de->inode_nr = inode_nr;
	strcpy(new_de->name, filename);

	/* write dir block -- ROOT dir block */
	WR_SECT(dir_inode->i_dev, dir_blk0_nr + i); // 这里写，是把 fsbuf写入到 i_dev分区的 第 dir_blk0_nr + i 个扇区中；
	/* update dir inode */
