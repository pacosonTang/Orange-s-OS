...
LABEL_START:			; <--- 从这里开始 *************
	...
	; 得到内存数 ， 这个需要好好研究一下 [ ready for studying ]
...
LABEL_SEARCH_IN_ROOT_DIR_BEGIN:
	...
	mov	ax, BaseOfKernelFile		; BaseOfKernelFile  equ	08000h	; KERNEL.BIN 被加载到的位置 ----  段地址
...
LABEL_FILENAME_FOUND:			; 找到 KERNEL.BIN 后便来到这里继续
	...
	push	eax
	mov	eax, [es : di + 01Ch]		; ┓根目录条目offset(28)=Dir_FileSize文件大小
	mov	dword [dwKernelSize], eax	; ┛保存 KERNEL.BIN 文件大小
	pop	eax

	add	di, 01Ah		; di -> 首 Sector
	mov	cx, word [es:di]; 根目录条目offset(26)=Dir_FirstClus 此条目对应的开始簇号
	push	cx			; 保存此 Sector 在 FAT 中的序号
	add	cx, ax			; 加上根目录区扇区数量
	add	cx, DeltaSectorNo	; 这时 cl 里面是 LOADER.BIN 的起始扇区号 (从 0 开始数的序号)，我们也叫全局扇区号
	mov	ax, BaseOfKernelFile ; BaseOfKernelFile    equ	 08000h	; KERNEL.BIN 被加载到的位置 ----  段地址
	mov	es, ax			; es <- BaseOfKernelFile
	mov	bx, OffsetOfKernelFile	; bx <- OffsetOfKernelFile , OffsetOfKernelFile  equ 0h	; KERNEL.BIN 被加载到的位置 ---- 偏移地址
								; 于是, es:bx = BaseOfKernelFile:OffsetOfKernelFile = BaseOfKernelFile * 10h + OffsetOfKernelFile
	mov	ax, cx			; ax <- Sector 号(全局扇区号)
...
LABEL_FILE_LOADED:
	...
	mov	dh, 1			; "Ready." , ready 表示吧loader.bin 文件已经加载到 以9000h:1000 为起始地址的内存空间去了；
	call	DispStrRealMode		; 显示字符串
...
; BaseOfLoaderPhyAddr equ	BaseOfLoader * 10h ;
	...
	push	szMemChkTitle
	call	DispStr ; 显示一个字符串， defined in lib.inc
	add	esp, 4
