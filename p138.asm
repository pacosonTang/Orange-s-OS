...
; _szRAMSize:	db "RAM size:", 0
	...
	; 下面这段代码应该是新加的,相对于p132.as,m而言，将内核转移到正确的内存地址 [add]
	call	InitKernel
	...
	jmp	SelectorFlatC:KernelEntryPointPhyAddr	; 正式进入内核 打印字母 'K '*

	; 内核文件 kernel.asm 如下所示： 
...
; BaseOfKernelFilePhyAddr	equ	BaseOfKernelFile * 10h
        ...
        mov   cx, word [BaseOfKernelFilePhyAddr+2Ch] ; ecx <- (pELFHdr->e_phnum) offset(44)=e_phnum; 程序头表 有多少个条目
        movzx ecx, cx ; 即ecx高16=0，低16位=cx                               ;/
        mov   esi, [BaseOfKernelFilePhyAddr + 1Ch]  ; esi <- pELFHdr->e_phoff, e_phoff: 程序头表在文件中的偏移量。
        add   esi, BaseOfKernelFilePhyAddr  ; esi<-OffsetOfKernel+pELFHdr->e_phoff, 程序头表在文件中的物理地址;(基地址+偏移量)
											; 因为ELF文件的结构为(依地址低->高)：ELF头 》程序头表（多个程序头） 》 多个节 》 节头表
.Begin:
        mov   eax, [esi + 0]				; 第一个程序头内结构体成员值，offset(0)=程序头所描的段类型
        cmp   eax, 0                      	; PT_NULL
        jz    .NoAction
        push  dword [esi + 010h]   			; offset(16)=p_filesz : 段在文件中的长度
        mov   eax, [esi + 04h]          	; offset(4)=p_offset: 段的第一个字节在文件中的偏移
        add   eax, BaseOfKernelFilePhyAddr	; | memcpy((void*)(pPHdr->p_vaddr), 段的第一个字节在文件中的物理地址；
        push  eax		    				; src  ; | uchCode + pPHdr->p_offset,
        push  dword [esi + 08h]     		; dst  ; | pPHdr->p_filesz; offset(8)=p_vaddr: 段的第一个字节在内存中的虚拟地址
        call  MemCpy                      	; | line_575 define
        add   esp, 12                     	; /
.NoAction:
        add   esi, 020h               	    ; esi += pELFHdr->e_phentsize ;每个程序头结构=32字节；读取下一个程序头结构
        dec   ecx
        jnz   .Begin
